...
const quizQuestions = [
  {
    question: 'What is a common sign of a phishing email?',
    options: ['Spelling mistakes', 'From a known company', 'Contains no links'],
    correctAnswer: 'Spelling mistakes',
    explanation: 'Scammers often use poor grammar and spelling mistakes in phishing emails.'
  },
  {
    question: 'What should you do if someone asks for your password?',
    options: ["Give it if it's urgent", 'Never share it', 'Write it down'],
  ...
  {
    question: 'How should you verify a suspicious message?',
    options: ['Call the sender directly', 'Reply to the message', 'Ignore it'],
    correctAnswer: 'Call the sender directly',
    explanation: 'Always verify requests by contacting the official organization.'
  },
  {
    question: 'What should you do if you suspect fraud?',
    options: ['Report it', 'Ignore it', 'Click the links to check'],
    correctAnswer: 'Report it',
    explanation: 'Reporting scams helps prevent others from becoming victims.'
  }
]
...
const QuizScreen = (): JSX.Element => {
  ...
  const progressPercentage: number = ((currentQuestion + 1) / quizQuestions.length) * 100
  ...
  const handleNext = async (): Promise<void> => {
    if (currentQuestion + 1 < quizQuestions.length) {
      ...
      try {
        await AsyncStorage.setItem(`quizProgress_${courseId}`, JSON.stringify(100))
        await AsyncStorage.setItem(`quizScore_${courseId}`, JSON.stringify(finalScore))
  ...
  return (
    <View className="flex-1 bg-gray-100 px-4 py-6">
      <View className="flex-row justify-between items-center">
        ...
        <View className="bg-orange-200 px-4 py-2 rounded-full">
          <Text className="text-gray-800 font-bold">{score} points</Text>
        </View>
      </View>

      <Text className="text-lg font-semibold mt-4">Lesson {currentQuestion + 1}/5</Text>
      <ProgressBar progress={progressPercentage} />

      <TouchableOpacity
        className="mt-4 border border-gray-400 p-3 rounded-lg flex-row items-center justify-center"
        onPress={handleListen}
      >
        <Text className="text-lg font-semibold">Listen</Text>
      </TouchableOpacity>
      ...
      {showExplanation ? (
        <View className="mt-6">
          <Text className="text-lg font-bold">{isCorrect ? '✅ Correct!' : '❌ Not quite!'}</Text>
          <Text className="mt-2 text-gray-600">{quizQuestions[currentQuestion].explanation}</Text>
          <TouchableOpacity className="mt-4 bg-blue-900 py-3 px-4 rounded-lg" onPress={handleNext}>
        ...
        <View className="mt-6">
          <Text className="text-lg font-bold mb-4">{quizQuestions[currentQuestion].question}</Text>
          {quizQuestions[currentQuestion].options.map((option, index) => (
            <TouchableOpacity
              key={index}
              className={`p-3 mb-2 rounded-lg ${
                selectedAnswer === option ? 'bg-orange-300' : 'bg-orange-100'
              }`}
              onPress={() => handleAnswer(option)}
            >
